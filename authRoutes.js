...
router.post("/register", async (req, res) => {
    try {
      ...
      if (!data) {
        ...
        let regToken = jwt.sign({email: req.body.email},process.env.REG_SECRET_KEY);
            await db.collection("users").insertOne({ firstname:req.body.firstname, lastname:req.body.lastname, email:req.body.email, password:req.body.password, active:false, regToken});
            ...
            let mailDetails = await mailTransporter.sendMail({
                ...
                html: `<div>
                    <h3>Thanks for Registering with Blog-Basic.. Click on the below link to activate your account :)</h3>
      ...
      if (data) {
        ...
        let mailTransporter = nodeMailer.createTransport({
         ...
         let mailDetails = await mailTransporter.sendMail({
          ...
          html: `<div>
                    <h3>If You have requested for Password Reset in Blog-Basic, Click the following link to reset your password.If not requested, then just ignore this mail</h3>
                    <a href="${feUrl+"users/reset/"+randomString}">RESET PASSWORD</a>
                  </div>`,
        })   
        console.log(mailDetails)
        res.status(200).json({
  ...
  router.post("/reset/:randomString", async (req, res) => {
    try {
      ...
      let db = await client.db("Blog-Basic");
